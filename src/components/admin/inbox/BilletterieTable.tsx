...
import { Eye, Calendar, Users, Plane, FileDown, CheckCircle } from "lucide-react";
  ...
  if (requests.length === 0) {
    return (
      <div className="text-center py-12 bg-white rounded-lg border border-slate-200">
        ...
        <p className="text-slate-500">Aucune demande de billetterie</p>
      </div>
    );
  }
          ...
          <table className="w-full">
            <thead className="bg-primary text-white">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Passagers</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Compagnie</th>
              ...
              {requests.map((req, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm text-slate-600">
                    {req.compagnie || <span className="text-slate-400 italic">Non spécifié</span>}
                  </td>
        ...
        {requests.map((req, index) => (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              ...
              <Plane size={24} className="text-primary/30 flex-shrink-0" />
            </div>

            <div className="space-y-2 text-sm border-t border-slate-100 pt-3">
              <div className="flex justify-between">
                <span className="text-slate-600">Type:</span>
                <span className="font-medium text-slate-900">{req.tripType}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-slate-900">
                  {req.nombreAdultes} adulte(s), {req.nombreEnfants} enfant(s)
                </span>
              </div>
              {req.compagnie && (
                <div className="flex justify-between">
                  <span className="text-slate-600">Compagnie:</span>
                  <span className="font-medium text-slate-900">{req.compagnie}</span>
                </div>
              )}
              {req.besoinVisa && (
                <div className="flex justify-between">
                  <span className="text-slate-600">Visa:</span>
                  <span className="font-medium text-slate-900">{req.besoinVisa}</span>
                </div>
              )}
            </div>
...
export default BilletterieTable;
