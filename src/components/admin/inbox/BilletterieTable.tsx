...
const formatPhoneForWhatsApp = (phone: string): string => {
  ...
  // If starts with 0, replace with 213 (Algeria country code)
...
const generateWhatsAppMessage = (req: BilletterieRequest): string => {
  const clientName = `${req.personalInfo.prenom} ${req.personalInfo.nom}`;
  return encodeURIComponent(
    `Bonjour ${clientName}, c'est House of Travel. Nous avons bien reçu votre demande pour Billetterie. Comment pouvons-nous vous aider ?`
  );
};
            ...
            {requests.map((req, index) => (
              ...
              >
                ...
                <td className={`px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm text-slate-600 whitespace-nowrap ${req.completed ? "opacity-60" : ""}`}>
                  {req.compagnie || <span className="text-slate-400 italic">N/A</span>}
                </td>
